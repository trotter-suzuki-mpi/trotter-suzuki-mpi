...
class State(Lattice):
    ...
    def __init__(self, grid=Lattice(), periods=None, p_real=None,
                 p_imag=None, pb_real=None, pb_imag=None):

        if not isinstance(grid, Lattice):
            print("First parameter is not a instance of Lattice class.")
            return

        Lattice.__init__(self, grid.length_y, grid.length_x,
                         grid.dim_y, grid.dim_x)
        if periods is None:
            self.periods = [0, 0]
        else:
    ...
    def init_state(self, function, functionb=None):
        ...
        if self.p_imag is None:
            self.p_imag = np.zeros((self.dim_y, self.dim_x))

        for i in range(0, self.dim_y):
            for j in range(0, self.dim_x):
                self.p_real[i, j] = np.real(function(self.delta_y *
                                                     (i - self.dim_y * 0.5),
                                                     self.delta_x *
                                                     (j - self.dim_x * 0.5)))
                self.p_imag[i, j] = np.imag(function(self.delta_y *
                                                     (i - self.dim_y * 0.5),
                                                     self.delta_x *
                                                     (j - self.dim_x * 0.5)))

        if functionb is not None:

            if self.pb_real is None:
                self.pb_real = np.zeros((self.dim_y, self.dim_x))

            if self.pb_imag is None:
                self.pb_imag = np.zeros((self.dim_y, self.dim_x))

            for i in range(0, self.dim_y):
                for j in range(0, self.dim_x):
                    self.pb_real[i, j] = np.real(functionb(self.delta_y *
                                                           (i-self.dim_y*0.5),
                                                           self.delta_x *
                                                           (j-self.dim_x*0.5)))
                    self.pb_imag[i, j] = np.imag(functionb(self.delta_y *
                                                           (i-self.dim_y*0.5),
                                                           self.delta_x *
                                                           (j-self.dim_x*0.5)))
    ...
    def calculate_squared_norm_b(self):
        """Function for calculating the squared norm of the state. """
        if self.pb_real is not None and self.pb_imag is not None:
            return Norm2(self.pb_real, self.pb_imag,
    ...
    def calculate_squared_norm_2(self):
        if self.pb_real is not None and self.pb_imag is not None:
            return Norm2_2GPE(self.p_real, self.p_imag, self.pb_real,
                              self.pb_imag, self.delta_x, self.delta_y)
        else:
            print("Second wave function is not defined.")
    ...
    def calculate_total_energy(self, hamiltonian):
        ...
        if not isinstance(hamiltonian, Hamiltonian):
            ...
            return H(self.p_real, self.p_imag, hamiltonian.mass,
                     hamiltonian.coupling_a, hamiltonian.external_pot,
    ...
    def calculate_kinetic_energy(self, hamiltonian):
        ...
        if not isinstance(hamiltonian, Hamiltonian):
            ...
            return K(self.p_real, self.p_imag, hamiltonian.mass,
                     self.delta_x, self.delta_y)
    ...
    def calculate_rotational_energy(self, hamiltonian):
        ...
        if not isinstance(hamiltonian, Hamiltonian):
            ...
            return Lz(self.p_real, self.p_imag, hamiltonian.angular_velocity,
                      hamiltonian.rot_coord_x, hamiltonian.rot_coord_y,
                      self.delta_x, self.delta_y)

    def calculate_total_energy_2GPE(self, hamiltonian):
        """Function for calculating the expectation value of the two component
        Hamiltonian.
        """
        if not isinstance(hamiltonian, Hamiltonian2Component):
            ...
            return H_2GPE(self.p_real, self.p_imag, self.pb_real, self.pb_imag,
                          hamiltonian.hamiltonian1.mass,
                          hamiltonian.hamiltonian2.mass,
                          [hamiltonian.hamiltonian1.coupling_a,
                           hamiltonian.hamiltonian2.coupling_a,
                           hamiltonian.hamiltonian1.coupling_ab,
                           np.real(hamiltonian.omega),
                           np.imag(hamiltonian.omega)],
                          hamiltonian.hamiltonian1.external_pot,
                          hamiltonian.hamiltonian2.external_pot,
                          hamiltonian.hamiltonian1.angular_velocity,
                          hamiltonian.hamiltonian1.rot_coord_x,
                          hamiltonian.hamiltonian1.rot_coord_y,
                          self.delta_x, self.delta_y)


class Hamiltonian(Lattice):

    def __init__(self, grid=Lattice(), coupling_a=0., coupling_ab=0.,
                 mass=1., angular_velocity=0., rot_coord_y=None,
                 rot_coord_x=None, external_pot=None):
        if not isinstance(grid, Lattice):
            print("First parameter is not a instance of Lattice class.")
            return
    ...
    def init_external_pot(self, function):
        ...
        for i in range(0, self.dim_y):
            for j in range(0, self.dim_x):
                self.external_pot[i, j] = function(self.delta_y *
                                                   (i - self.dim_y * 0.5),
                                                   self.delta_x *
                                                   (j - self.dim_x * 0.5))
...
class Hamiltonian2Component(object):
    ...
    def __init__(self, hamiltonian1=Hamiltonian(), hamiltonian2=Hamiltonian(),
                 omega=0.):
...
class Solver(State):
    ...
    def __init__(self, grid, hamiltonian, state=State(), imag_time=False,
                 iterations=100, delta_t=1e-4, kernel_type='cpu'):
        ...
        if isinstance(hamiltonian, Hamiltonian):
            ...
            print("Second parameter is not a instance of Hamiltonian or "
                  "Hamiltonian2Component classes.")
            return
        ...
        State.__init__(self, grid, state.periods, state.p_real, state.p_imag,
                       state.pb_real, state.pb_imag)
        Lattice.__init__(self, grid.length_y, grid.length_x,
                         grid.dim_y, grid.dim_x)
    ...
    def evolve(self, imag_time=None, iterations=None, delta_t=None,
               kernel_type=None):

        if imag_time is None:
        ...
        if self.single_component:
            solver(self.p_real, self.p_imag, self.mass_a, self.coupling_a,
                   self.external_pot_a, self.angular_velocity,
                   self.rot_coord_x, self.rot_coord_y,
                   self.delta_x, self.delta_y, evo_delta_t, evo_iterations,
                   evo_kernel_type, self.periods, evo_imag_time)
        else:
            solver_2GPE(self.p_real, self.p_imag, self.pb_real, self.pb_imag,
                        self.mass_a, self.mass_b,
                        [self.coupling_a, self.coupling_b, self.coupling_ab],
                        self.external_pot_a, self.external_pot_b,
                        self.angular_velocity, self.rot_coord_x,
                        self.rot_coord_y, self.delta_x, self.delta_y,
                        evo_delta_t, evo_iterations, evo_kernel_type,
                        self.periods, evo_imag_time)
