...
void HybridKernel::run_kernel() {
    ...
    #pragma omp parallel default(shared) private(block_start)
    {
        ...
        for (block_start = block_height - 2 * halo_y; block_start < (int)last_band; block_start += block_height - 2 * halo_y) {
            process_band(0., 0., 0., 0., tile_width, block_width, block_height, halo_x, block_start, block_height, halo_y, block_height - 2 * halo_y, a, b, coupling_const, external_pot_real, external_pot_imag, p_real[sense], p_imag[sense], NULL, NULL, p_real[1 - sense], p_imag[1 - sense], inner, sides, imag_time);
        }
    }
    #pragma omp barrier
    sense = 1 - sense;
...
void HybridKernel::run_kernel_on_halo() {
    ...
    if (tile_height <= block_height) {
        ...
        inner = 1;
        sides = 1;
        process_band(0., 0., 0., 0.,tile_width, block_width, block_height, halo_x, 0, tile_height, 0, tile_height, a, b, coupling_const, external_pot_real, external_pot_imag, p_real[sense], p_imag[sense], NULL, NULL, p_real[1 - sense], p_imag[1 - sense], inner, sides, imag_time);
    }
    else {
#ifdef _OPENMP
        int block_start;
        #pragma omp parallel default(shared) private(block_start)
        {
            #pragma omp sections nowait
            {
                #pragma omp section
                {
                    ...
                    inner = 1;
                    sides = 1;
                    process_band(0., 0., 0., 0.,tile_width, block_width, block_height, halo_x, 0, block_height, 0, block_height - halo_y, a, b, coupling_const, external_pot_real, external_pot_imag, p_real[sense], p_imag[sense], NULL, NULL, p_real[1 - sense], p_imag[1 - sense], inner, sides, imag_time);
                ...
                #pragma omp section
                {
                    ...
                    inner = 1;
                    sides = 1;
                    process_band(0., 0., 0., 0.,tile_width, block_width, block_height, halo_x, block_start, tile_height - block_start, halo_y, tile_height - block_start - halo_y, a, b, coupling_const, external_pot_real, external_pot_imag, p_real[sense], p_imag[sense], NULL, NULL, p_real[1 - sense], p_imag[1 - sense], inner, sides, imag_time);
                }
            }
