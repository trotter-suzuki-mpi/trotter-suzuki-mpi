...
void solver(Lattice *grid, State *state,
  ...
  Lattice *padded_grid = new Lattice(dimx*grid->delta_x, dimy*grid->delta_y, dimx, dimy, dimx, dimy, grid->periods);
    ...
    ini_external_pot(external_pot_real, external_pot_imag, external_pot, delta_t, dimx, dimy, halo_x, halo_y, grid->global_dim_x, grid->global_dim_y, grid->periods, imag_time);
    
    double h_a, h_b;
    if(imag_time) {
		h_a = cosh(delta_t / (4. * particle_mass * grid->delta_x * grid->delta_y));
		h_b = sinh(delta_t / (4. * particle_mass * grid->delta_x * grid->delta_y));	
    }
    else {
		h_a = cos(delta_t / (4. * particle_mass * grid->delta_x * grid->delta_y));
		h_b = sin(delta_t / (4. * particle_mass * grid->delta_x * grid->delta_y));
    }
	
	//launch kernel
	trotter(padded_grid, padded_state, h_a, h_b, coupling_const, external_pot_real, external_pot_imag, delta_t, iterations, omega, rot_coord_x, rot_coord_y, kernel_type, norm, imag_time);
